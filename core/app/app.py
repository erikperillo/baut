...
class App:
    ...
    def __init__(self, struct_dir="", key="", name=""):
        ...
        if struct_dir != "":
            #looking in specified dir to open key file
            f = open(struct_dir + "/" + KEY_FILENAME,"r")
            self.key = f.read()
            f.close()
        else:
            self.key = key
        ...
        if osp.isfile(struct_dir + "/" + OPT_CONFIG_FILENAME):
            sp.Popen([struct_dir + "/" + OPT_CONFIG_FILENAME]).wait() != 0:
...
class Extractor(App):
    ...
    def __init__(self, filter_script, struct_dir="", name=""):
