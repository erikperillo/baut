...
def extract():
    ...
    run_dir = oarg.Oarg("-r --run-dir", "", "directory of applications run")
    vars_path = oarg.Oarg("-v --vars", os.path.join(file_dir, "vars", "vars.csv"), 
                          "vars .csv file path")
    ...
    for oa in vars_path, vars_names, run_dir:
        if not oa.found:
            error("argument of keyword '-%s' must be passed" % oa.keywords[0])
    ...
    #getting states dirs
    states_dirs = [d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d)) \
                   and d.startswith("state_")]
